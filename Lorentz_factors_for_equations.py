...
#Система двух обыкновенных диффур на M2 и Y, где
#M2 - квадрат альфвеновского числа Маха, Y - обезразмеренный магнитный поток
#Система для новых интегралов движения Omega(Psi) и L(Psi)
def f(X, x_r):
   Y = X[0]
   M2 = X[1]
...
#Лоренц-фактор из уравнения Грэда-Шафранова
...
#Гидродинамический лоренц-фактор через МГД
...
#Гидродинамический лоренц-фактор с учётом epsilon
...
#Гидродинамический лоренц-фактор в случае Omega=const
...
#Построение профиля локальной замагниченности
def L(Y):
...
N = 100000 #Число пространственных точек определения лоренц-фактора

#Введём начальные условияx
rho_0 = 0.001
#M2_0 = 3
M2_0 = 3
Y_0 = 1/2*Gamma_in/M2_0*rho_0**2
ics=[Y_0, M2_0]
#rho = np.linspace(rho_0,53.9,N) #расстояние от оси джета в R_L
rho = np.linspace(rho_0,53.9,N) #расстояние от оси джета в R_L
...
#Вычислим значения электромагнитных полей
Psii = Psi(Y)
Omega_FF = Omega_F(Y)
II = I(rho, Y, M2)
K = len(Y)
rho_MHD = np.zeros(K-1)
#Конфигурация электромагнтиных полей через вектора
B_p = np.zeros(K-1)
E = np.zeros(K-1)
B_phi = np.zeros(K-1)
for j in range(K):
